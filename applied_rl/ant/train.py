...
class DDPG:
    ...
    def update(self, transition):
        ...
        if len(self.replay_buffer) > BATCH_SIZE * 16:
            ...
            q_next = self.target_critic_1(next_state, self.target_actor(next_state))
            q_target = reward + GAMMA * q_next
            
            # Critic update
            self.critic_1.zero_grad()

            q_cur = self.critic_1(state, action)
            
            value_loss = F.mse_loss(q_cur, q_target)
            value_loss.backward()
            ...
            policy_loss = -self.critic_1(state, self.actor(state))
...
if __name__ == "__main__":
    ...
    for i in range(TRANSITIONS):
        ...
        ddpg.update((state, action, next_state, reward, done))
        
        state = next_state if not done else env.reset()
        
        if (i + 1) % (TRANSITIONS//100) == 0:
            rewards = evaluate_policy(test_env, ddpg, 5)
